...
describe("TrustedRelayerOracle Contract", function () {
    ...
    beforeEach(async function() {
        ...
        oracle = await upgrades.deployProxy(OracleFactory, [], { kind: 'transparent' }) as unknown as TrustedRelayerOracle;
    ...
    describe("Relayer Management", function () {
        it("should allow admin to add a relayer", async function () {
            ...
            await (await oracle.connect(owner).addRelayer(relayer1.address, "https://api.example.com/data")).wait();
        ...
        it("should NOT allow a non-admin to add a relayer", async function () {
            await expect(oracle.connect(user1).addRelayer(relayer1.address, "https://api.example.com/data"))
                .to.be.reverted;
        });
    });
    ...
    describe("Price Updates", function () {
        ...
        it("should NOT allow a non-relayer to update a price", async function () {
            await expect(oracle.connect(user1).updatePrice(pairId, price))
                .to.be.reverted;
        });
