...
describe("AndeOracleAggregator Contract", function () {
    ...
    let owner: ethers.Wallet, user1: ethers.Wallet, relayer1: ethers.Wallet, relayer2: ethers.Wallet;
    ...
    beforeEach(async function() {
        ...
        aggregator = await upgrades.deployProxy(AggregatorFactory, [], { kind: 'transparent' }) as unknown as AndeOracleAggregator;
        await aggregator.waitForDeployment();

        // Desplegar Oráculos fuente usando deployProxy
        const OracleFactory = await ethers.getContractFactory("TrustedRelayerOracle", owner);
        source1 = await upgrades.deployProxy(OracleFactory, [], { kind: 'transparent' }) as unknown as TrustedRelayerOracle;
        await source1.waitForDeployment();
        source2 = await upgrades.deployProxy(OracleFactory, [], { kind: 'transparent' }) as unknown as TrustedRelayerOracle;
        await source2.waitForDeployment();

        // Añadir relayers a los oráculos fuente
        await (await source1.connect(owner).addRelayer(relayer1.address, "source1")).wait();
        await (await source2.connect(owner).addRelayer(relayer2.address, "source2")).wait();
    });
    ...
    describe("Source Management", function () {
        it("should allow the owner to add a source", async function () {
            await (await aggregator.connect(owner).addSource(pairId, await source1.getAddress(), 10000, 1, "Source 1")).wait();
        ...
        it("should NOT allow a non-owner to add a source", async function () {
            await expect(aggregator.connect(user1).addSource(pairId, await source1.getAddress(), 10000, 1, "Source 1"))
                .to.be.revertedWithCustomError(aggregator, "OwnableUnauthorizedAccount").withArgs(user1.address);
        });
    });
    ...
    describe("Price Aggregation", function () {
        it("should return a price from a single source", async function () {
            ...
            await (await source1.connect(relayer1).updatePrice(pairId, price1)).wait();
            await (await aggregator.connect(owner).addSource(pairId, await source1.getAddress(), 10000, 1, "Source 1")).wait();
        ...
        it("should aggregate prices from multiple sources", async function () {
            ...
            await (await source1.connect(relayer1).updatePrice(pairId, price1)).wait();
            await (await source2.connect(relayer2).updatePrice(pairId, price2)).wait();

            await (await aggregator.connect(owner).addSource(pairId, await source1.getAddress(), 5000, 1, "Source 1")).wait(); // 50% weight
            await (await aggregator.connect(owner).addSource(pairId, await source2.getAddress(), 5000, 1, "Source 2")).wait(); // 50% weight
        ...
        it("should exclude outliers from aggregation", async function () {
            ...
            const OracleFactory = await ethers.getContractFactory("TrustedRelayerOracle", owner);
            const source3 = await upgrades.deployProxy(OracleFactory, [], { kind: 'transparent' }) as unknown as TrustedRelayerOracle;
            await source3.waitForDeployment();
            await (await source3.connect(owner).addRelayer(owner.address, "source3")).wait();

            // Update prices
            await (await source1.connect(relayer1).updatePrice(pairId, price1)).wait();
            await (await source2.connect(relayer2).updatePrice(pairId, price2)).wait();
            await (await source3.connect(owner).updatePrice(pairId, price3_outlier)).wait();

            // Add sources to aggregator
            await (await aggregator.connect(owner).addSource(pairId, await source1.getAddress(), 3333, 1, "Source 1")).wait();
            await (await aggregator.connect(owner).addSource(pairId, await source2.getAddress(), 3333, 1, "Source 2")).wait();
            await (await aggregator.connect(owner).addSource(pairId, await source3.getAddress(), 3334, 1, "Source 3")).wait();

            const [price, confidence, sourcesUsed] = await aggregator.getPriceWithConfidence(pairId);
