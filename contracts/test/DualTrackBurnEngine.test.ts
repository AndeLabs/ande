...
describe("DualTrackBurnEngine", function () {
  ...
  let owner: HardhatEthersSigner, burner: HardhatEthersSigner, otherAccount: HardhatEthersSigner;
  ...
  beforeEach(async function () {
    ...
    await andeToken.connect(owner).mint(await dualTrackBurnEngine.getAddress(), initialBurnableAmount);
  });
  ...
  describe("Deployment and Initialization", function () {
    it("Should set the right admin, burner, and token address", async function () {
      ...
      expect(await dualTrackBurnEngine.hasRole(ADMIN_ROLE, owner.address)).to.be.true;
      expect(await dualTrackBurnEngine.hasRole(BURNER_ROLE, burner.address)).to.be.true;
      expect(await dualTrackBurnEngine.andeToken()).to.equal(await andeToken.getAddress());
    });
  });
  ...
  describe("Impulsive Burn", function () {
    it("Should allow the burner to burn tokens", async function () {
      ...
      const initialBalance = await andeToken.balanceOf(await dualTrackBurnEngine.getAddress());
      const initialTotalSupply = await andeToken.totalSupply();

      await dualTrackBurnEngine.connect(burner).impulsiveBurn(burnAmount);

      const finalBalance = await andeToken.balanceOf(await dualTrackBurnEngine.getAddress());
    ...
    it("Should NOT allow a non-burner to burn tokens", async function () {
      ...
      ).to.be.revertedWithCustomError(dualTrackBurnEngine, "AccessControlUnauthorizedAccount").withArgs(otherAccount.address, BURNER_ROLE);
    });

    it("Should revert if burn amount is zero", async function () {
        await expect(dualTrackBurnEngine.connect(burner).impulsiveBurn(0))
            .to.be.revertedWith("Burn amount must be positive");
    });

    it("Should revert if burn amount exceeds balance", async function () {
        const balance = await andeToken.balanceOf(await dualTrackBurnEngine.getAddress());
        const burnAmount = balance + ethers.parseUnits("1", 18);

        await expect(dualTrackBurnEngine.connect(burner).impulsiveBurn(burnAmount))
  ...
  describe("Scheduled Burn", function () {
    it("Should NOT allow burning before the schedule period has passed", async function () {
      await expect(dualTrackBurnEngine.scheduledBurn())
        .to.be.revertedWith("Scheduled burn period not yet passed");
    });
    ...
    it("Should reset the lastScheduledBurnTimestamp", async function () {
        ...
        const beforeTimestamp = await dualTrackBurnEngine.lastScheduledBurnTimestamp();
        
        await dualTrackBurnEngine.scheduledBurn();

        const afterTimestamp = await dualTrackBurnEngine.lastScheduledBurnTimestamp();
