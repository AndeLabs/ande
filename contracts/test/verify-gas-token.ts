...
describe("Gas Token Verification", function () {
  it("should use the native 'aande' token for gas fees", async function () {
    ...
    const provider = new ethers.JsonRpcProvider("http://ev-reth-sequencer:8545");
    ...
    if (!privateKey) {
      throw new Error("PRIVATE_KEY not found in environment variables. Please check your setup.");
    }
    ...
    console.log(`💰 Initial balance: ${ethers.formatEther(initialBalance)} aande`);

    // We expect the genesis account to be funded.
    expect(initialBalance).to.be.gt(0, "Wallet has no initial balance. Check genesis funding.");
    ...
    expect(finalBalance).to.be.lt(initialBalance, "Final balance should be less than initial balance after paying for gas.");
