 ...
 */
contract TrustedRelayerOracle is
    Initializable,
    AccessControlUpgradeable,
    PausableUpgradeable,
    RateLimiter // IOracle, CircuitBreaker removed
{
    ...
    function addRelayer(address relayer, string calldata endpoint) external onlyRole(ADMIN_ROLE) {
        ...
        relayers[relayer] = RelayerInfo({isActive: true, totalUpdates: 0, lastUpdate: 0, endpoint: endpoint});
        totalRelayers++;
    ...
    function updatePrice(bytes32 pairId, uint256 newPrice) public onlyRole(RELAYER_ROLE) whenNotPaused rateLimit {
        PriceValidator.PriceData memory priceToValidate =
            PriceValidator.PriceData({price: newPrice, timestamp: block.timestamp, confidence: 10000});
    ...
    function updatePricesBatch(bytes32[] calldata pairIds, uint256[] calldata newPrices)
        external
    ...
    function getPriceWithMetadata(bytes32 pairId)
        external
        view
        returns (uint256 price, uint256 timestamp, bool isStale, address source)
    {
        PriceData memory data = prices[pairId];
